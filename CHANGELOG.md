...
- **MQTT**: Updated the `startup_time` diagnostic sensor to use the modern `uptime` device class and `mdi:clock-start` icon, following Home Assistant's latest standards.
...
## [4.2.2] - 2026-04-11
### Changed
- **CI/CD**: Converted the Publish workflow from a `workflow_run` trigger to a reusable workflow (`workflow_call`), ensuring workflow changes on `beta` are tested with the correct version instead of always using `main`'s copy.
...
- **Contributing Guide**: Added `CONTRIBUTING.md` (GitHub-standard contributor guide) covering architecture, project structure, dev setup, code standards, release process, and how to submit PRs.
...
## [3.0.3] - 2026-02-14
### Added
- **Security Policy**: Added `SECURITY.md` to the repository to provide clear guidelines on reporting vulnerabilities.
- **Documentation**: Minor documentation improvements.

## [3.0.2] - 2026-02-14
### Added
- **Docker Healthcheck**: Added health monitoring for both HA addon and standalone Docker modes. Enables the HA Supervisor watchdog toggle to automatically restart the addon if it becomes unresponsive.
- **Release Automation**: Added a GitHub Actions workflow that automatically creates GitHub Releases with notes from the CHANGELOG when a version tag is pushed.

## [3.0.1] - 2026-02-13
### Changed
- **Dependency Updates**: Updated `ruff` to version 0.15.1 for improved linting and code quality checks.
...
## [2.3.5] - 2026-01-24
### Added
- **Dependency Automation**: Performance and reliability improvements.

### Fixed
- **Test Suite Maintenance**: Restored the test suite to exactly 34 tests. Fixed a missing `threading` import and resolved platform-specific path assertion failures in the configuration tests.

## [2.3.4] - 2026-01-24
### Added
- **Advanced Repository Automation**: Fully implemented **Renovate Bot** with a custom configuration to track Home Assistant's `build.yaml` base images, Python requirements, Dockerfiles, and GitHub Actions.
- **Deep Directory Scanning**: Configured Renovate to explicitly search and monitor the `tests/` directory, surfacing previously hidden dependencies in `requirements-test.txt` and `Dockerfile.test`.
- **Automated Update Grouping**: Integrated best-practice presets for separating major releases and combining patch/minor updates to minimize PR noise.

### Fixed
- **CI/CD Maintenance**: Resolved a deprecation warning in the test workflow by updating the `codecov-action` input from `file` to `files`.

## [2.3.3] - 2026-01-24
### Added
- **Dependency Automation**: Initial integration of automated dependency tracking.

## [2.3.2] - 2026-01-24
### Security
- **GitHub Actions Hardening**: Added explicit `permissions: contents: read` to workflows to follow the principle of least privilege, reducing the attack surface in case of a compromised GITHUB_TOKEN.

## [2.3.1] - 2026-01-24
### Fixed
- **Dependency Synchronization**: Synchronized test dependencies with production (pinning `paho-mqtt` to `2.1.0`), ensuring the test suite validates against the exact same logic used in the live environment.
- **Documentation Polish**: Cleaned up outdated Python 3.11 references and "template" descriptions in `STATUS.md`, `README.md`, and `DOCKER_TESTING.md`.
- **CI/CD Alignment**: Updated GitHub Actions to trigger on `dev` branch pushes and verified Python 3.14 environment consistency.
...
### Changed
- **Modernized Core Logic**: Refactored `ReadConfig` and `ReadMeasurement` using modern Python idiomatic patterns, including `pathlib` for file operations and `fromisoformat` for date handling.
...
- **Improved Reliability**: Refined logging and configuration loading to be idempotent and more robust against environment variations.
